...
import re
from re_code import create_dynamic_regex,remove_trailing_alphabet
...
class DiscourseProcessor:
    def __init__(self, text: list, pattern: str):
        super().__init__()
        self.text = text
        self.regex_pattern = pattern
        self.usr_instance = USR()
        self.multi_word_markers = self.usr_instance.multi_word_markers
        self.markers = self.usr_instance.markers
        self.discourse_relation = self.usr_instance.discourse_relation
        self.convert_to_wx = WXC(order='utf2wx', lang='hin')
        self.updated_list = []
        self.hindi_dict = {}
    ...
    def combine_lines_with_prefix(lines):
        ...
        return process_discourse(new_lines)

    @staticmethod
    def convert_to_hindi(input_text):
        wx = WXC(order='wx2utf', lang='hin')
        return wx.convert(input_text)
    ...
    def get_ids(self, sent):
        matches = re.findall(self.regex_pattern, sent)
        return matches[0] if matches else None

    def to_check_same_ids(self, sent):
        matches = re.findall(self.regex_pattern, sent)
        return remove_trailing_alphabet(matches[0]) if matches else None
    ...
    list1=[]
    def parse_discourse_lines(self,lines):
        l1 = []
        l2 = []
        store = []
        # i=0
        ...
        for i in range(len(lines)-1):
            ...
            id1 = self.to_check_same_ids(sent1)
            id2 = self.to_check_same_ids(sent2)
            
            id_1 = self.get_ids(sent1)
            id_2 = self.get_ids(sent2)
            

            if id1==id2:
                # Check if "अगर" is present in sent2
                if ('कि अगर') not in sent2 and any(conj in sent1 for conj in ["अगर", "यदि", "यद्यपि"]):

                    m = i
                    sent1=lines[m].strip()
                    sent2=lines[m+1].strip()
                    if "तो" not in sent2 :
                        discourseMarkerParser.replaced_words_dict[id_1]= str(self.find_conjunction(sent1))
                        output_line=self.process_sentences(sent1, sent2, id_1, id_2).replace(' </S_id> </S_id> </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id> ] </S_id> </S_id>', ' </S_id> ]').replace(' </S_id> ] </S_id>',' </S_id> ]')
                        lines = self.replace_values_in_list(lines,sent1, sent2,output_line)
                        return self.process_discourse(lines)
                        
                    elif "तो" in sent2 :
                        return self.if_to(lines)
                    
                elif any(conj in sent2 for conj in ["अगर", "यदि", "यद्यपि"]):
                    ...
                    if sent3 and "तो" not in sent3 :
                        # #print("2")
                        #print(sent2)
                        id1 = self.to_check_same_ids(sent2)
                        id2 = self.to_check_same_ids(sent3)
                        
                        id_1 = self.get_ids(sent2)
                        id_2 = self.get_ids(sent3)
                        
                        if id1 == id2:
                            discourseMarkerParser.replaced_words_dict[id_1]= str(self.find_conjunction(sent1))
                            # #print(discourseMarkerParser.replaced_words_dict,'ciiiiiiiiii')
                            output_line = self.process_sentences(sent2, sent3, id_1, id_2).replace(' </S_id> </S_id> </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id> ] </S_id> </S_id>', ' </S_id> ]').replace(' </S_id> ] </S_id>',' </S_id> ]')
                            lines = self.replace_values_in_list(lines,sent2, sent3,output_line)
                            return self.process_discourse(lines)
                ...
                output_line = self.process_sentences(sent1, sent2, id_1, id_2).replace('अगर ', '').replace('यदि ', '').replace("यद्यपि ",'').replace(' </S_id> </S_id> </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id> ] </S_id> </S_id>', ' </S_id> ]').replace(' </S_id> ] </S_id>',' </S_id> ]')
                lines = self.replace_values_in_list(lines,sent1,sent2,output_line)
                if len(lines) > 2:
                    return self.process_discourse(lines)
                else:
                    output_line=output_line.replace(' </S_id> </S_id> ] </S_id> </S_id>', ' </S_id> ]').replace(' </S_id> ] </S_id>',' </S_id> ]')
                    output_line = self.process_output_line(output_line)
        ...
        while i<len(lines)-1:
            ...
            if "अगर" in sent2 or "यदि" in sent2 or "यद्यपि" in sent2 :
                ...
                sent3 = lines[i + 2].strip() if i + 2 < len(lines) else None
                discourseMarkerParser.replaced_words_dict[id_2]= str(self.find_conjunction(sent1))
                output_line_next = self.process_sentences(sent2, sent3, id_2, self.get_ids(sent3)) if sent3 else None
                lines = self.replace_values_in_list(lines,sent2,sent3,output_line_next)

                # Process with the current sentence and the next one
                discourseMarkerParser.replaced_words_dict[id_1]= str(self.find_conjunction(sent1))
                output_line_current = self.process_sentences(sent1, output_line_next , id_1, self.get_ids(output_line_next)) if output_line_next else None
                lines = self.replace_values_in_list(lines,sent1,output_line_next,output_line_current)

                return self.process_discourse(lines)

        # Normal processing if "अगर" is not present in sent2
            # discourseMarkerParser.replaced_words_dict[id_1]= str(find_conjunction(sent1))
            output_line = self.process_sentences(sent1, sent2, id_1, id_2).replace('अगर ', '').replace('यदि ', '').replace(' </S_id> </S_id> </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id>', ' </S_id>').replace(' </S_id> </S_id> ] </S_id> </S_id>', ' </S_id> ]').replace(' </S_id> ] </S_id>',' </S_id> ]')
            lines = self.replace_values_in_list(lines,sent1,sent2,output_line)
            if len(lines) > 2:
                if i == 0:
        ...
        for i in range(len(lines)):
            ...
            if "तो" in sent3:
                ...
                while b < len(lines):
                    ...
                    if sent4 and 'तो' not in sent4 and self.to_check_same_ids(sent3) == self.to_check_same_ids(sent4):
                        # #print("Processing sentences following 'तो' until IDs match at line:", b)
                        discourseMarkerParser.replaced_words_dict[self.get_ids(sent3)] = str(find_conjunction(sent3))
...
# 4b और छाता उड़ गया ।'''
    ...
    all_ids=[sentence['sentence_id'] for sentence in input['sentences']]
