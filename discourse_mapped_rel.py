   ...
   def set_marker_info(self):
       #set of single word markers
           ...
           self.multi_word_markers = {"nahIM wo","isake pariNAmasvarUpa",'isI kAraNa', 'isake viparIwa',
                             "isake alAvA", 'isake awirikwa', 'isake sAWa-sAWa','isake sAWa sAWa','isa kAraNa','isake bAvajZUxa','isake bAxa','bAxa meM'
                               }
           ...
           self.discourse_relation = {
                ...
                #    "yaxi" : "AvaSyakawApariNAma",
                   ...
                   'nA kevala': 'samuccaya',
                   'arWAwa':'arWAwa',
                   'xUsre SabxoM meM' :'arWAwa',
                   'bAxa meM':'uwwarkAla',
                   'isake bAxa':'uwwarkAla'
...
class discourseMarkerParser(USR):
    """
    Returns discourse relation between two sentences by traversing
    the first three words of both sentences.
    Child class of USR
    """
    replaced_words_dict = {}  # Dictionary to store replaced words with their IDs
    def __init__(self, sent1: str, sent2: str):
        ...
        #print(sent1_wx,'sent333')
        sent2_wx = self.convert_to_wx.convert(sent2[1])
        # discourseMarkerParser.replaced_words_dict = {}  # Dictionary to store replaced words with their IDs
    ...
    def run_process(self, sent1_id: str, sent1: str, sent2_id: str, sent2: str):
        ...
        self.id2 = sent2_id
        self.s2 = sent2
        self.res_s1, self.res_disc, self.res_s2, discourseMarkerParser.replaced_words_dict = self.set_result()  # Store replaced words in a dict
    ...
    def set_result(self):
        ...
        if disc_relation == "-1":
            ...
            if disc_relation == "-1":
                return self.s1.rstrip('.'), "NaN", self.s2.rstrip('.'), discourseMarkerParser.replaced_words_dict
            else:
                ...
                for i in range(len(s_w) - 1):
                    if s in self.markers or s in self.multi_word_markers:
                        discourseMarkerParser.replaced_words_dict[self.id2] = s  # Store replaced word with ID
                        self.s2 = self.s2.replace(s, '')
                    else:
                        s += ' ' + s_w[i + 1]

                return self.s1.rstrip('.').strip(' '), disc_relation, self.s2.strip('.'), discourseMarkerParser.replaced_words_dict
            ...
            for i in range(len(s_w) - 1):
                if s_1 in self.markers or s_1 in self.multi_word_markers:
                    discourseMarkerParser.replaced_words_dict[self.id1] = s_1  # Store replaced word with ID
            ...
            if disc_relation == "-1":
                ...
                if disc_pos == "0":
                    ...
                    return self.s1.rstrip('.').strip(' '), disc_relation, self.s2.rstrip('.').strip(), discourseMarkerParser.replaced_words_dict

    def get_results(self):
        # print(discourseMarkerParser.replaced_words_dict,'replaced')
        return self.res_s1, self.res_disc, self.res_s2, discourseMarkerParser.replaced_words_dict  # Return replaced words dictionary
